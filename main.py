...
# =========================================================
# Setup UI (Step1 / Step2)  ※rowは0〜4、Selectは1行1個
# =========================================================
...
class SetupStep2View(discord.ui.View):
    def __init__(self, st: dict):
        ...
        self.add_item(discord.ui.Button(label="戻る", style=discord.ButtonStyle.secondary, custom_id="setup:back", row=3))
        self.add_item(discord.ui.Button(label="作成（公開投稿）", style=discord.ButtonStyle.success, custom_id="setup:create", row=3))
...
# =========================================================
# Component router (slot/notify/break/del + setup wizard)
# =========================================================
@client.event
    ...
    try:
        ...
        if cid.startswith("setup:"):
            ...
            if cid == "setup:day:today":
                st["day_key"] = "today"
                await interaction.response.edit_message(embed=build_setup_embed(st), view=SetupStep1View(st))
                return
            ...
            if cid == "setup:next":
                st["step"] = 2
                await interaction.response.edit_message(embed=build_setup_embed(st), view=SetupStep2View(st))
                return
            if cid == "setup:back":
                st["step"] = 1
                await interaction.response.edit_message(embed=build_setup_embed(st), view=SetupStep1View(st))
            ...
            if cid == "setup:create":
                ...
                # slotsは「必須列中心」にする（列が増減してても死ににくい）
                slot_rows = []
                cur = start_dt
                while cur < end_dt:
                    slot_rows.append({
        ...
        # breaksel は View 側で処理する（ここでは何もしない）
        ...
        if cid.startswith("del:"):
            ...
            try:
                await db_to_thread(lambda: db_delete_slots(panel_id))
                # panel削除は day_keyで
                await db_to_thread(lambda: sb.table("panels").delete().eq("guild_id", guild_id).eq("day_key", day_key).execute())
            except Exception as e:
